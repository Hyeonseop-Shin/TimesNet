...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        pe.requires_grad = False    # "require_grad" in official code
    ...
    def forward(self, x):
        return self.pe[:, :x.size(1)]   # take only the length of x
...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = x.long()    # (month, day, weekday, hour, minute)
...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', droupout=0.1):
        ...
        self.temporal_embedding = TemporalEmbedding(d_model=d_model, embed_type=embed_type, freq=freq) \
                                if embed_type != 'timeF' \
                                else TimeFeatureEmbedding(d_model, embed_type=embed_type, freq=freq)
